...
float TextureWrap::mixValue = 0.3f;

int TextureWrap::another_texture_wrap() {
  ...
  string fragmentPath = FileSystem::getPath("src/3_Textures/texture_wrap/wrap_fs.glsl");
 ...
 */
GLFWwindow *TextureWrap::create_glfw_window() {
  // Create a windows
 ...
 * @param height
 */
void TextureWrap::framebuffer_size_callback(GLFWwindow *window, int width, int height) {
 ...
 * @param window
 */
void TextureWrap::processInput(GLFWwindow *window) {
...
void TextureWrap::check_shader(unsigned int shader, const string &type) {
  int success;
  char infoLog[512];
...
void TextureWrap::check_program(unsigned int shaderProgram) {
  int success;
  char infoLog[512];
...
void TextureWrap::setMixValue(float value) {
  if (value < 0) {
    mixValue = 0.0f;
    return;
  } else if (value > 1) {
  ...
  mixValue = value;
  cout << "a = " << mixValue << endl;
}

float TextureWrap::getMixValue() {
  return mixValue;
}
