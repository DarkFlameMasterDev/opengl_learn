 ...
 *
 *          -0.5f, -0.5f, 0.0f, 对应 （0，0，0）
 *          0.5f, -0.5f, 0.0f,  对应 （127,0,0）
 *          0.0f, 0.5f, 0.0f    对应 （0,127,0）
 */
...
int final_practice::create_a_triangle() {
  init_glfw();
  ...
  const char *vertexShaderSource = "#version 330 core\n"
                                   ...
                                   "   vertexPoint = vec4(aPos.x, aPos.y, aPos.z, 1.0);\n"
                                   "}\0";
 ...
 */
GLFWwindow *final_practice::create_glfw_window() {
  // Create a windows
 ...
 * @param height
 */
void final_practice::framebuffer_size_callback(GLFWwindow *window, int width, int height) {
 ...
 * @param window
 */
void final_practice::processInput(GLFWwindow *window) {
  if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS) {
    glfwSetWindowShouldClose(window, true);
  }
}

void final_practice::check_shader(unsigned int shader, const string &type) {
  int success;
  char infoLog[512];
...
void final_practice::check_program(unsigned int shaderProgram) {
  int success;
  char infoLog[512];
...
void final_practice::draw_frame(unsigned int shaderProgram, unsigned int VAO) {
