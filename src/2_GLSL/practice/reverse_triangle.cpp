...
#include "reverse_triangle.h"

using namespace std;

int reverse_triangle::create_a_reverse_triangle() {
  init_glfw();
 ...
 * initialize glfw
 */
void reverse_triangle::init_glfw() {
  // glfw initialize
  glfwInit();
 ...
 */
GLFWwindow *reverse_triangle::create_glfw_window() {
  // Create a windows
 ...
 * @param height
 */
void reverse_triangle::framebuffer_size_callback(GLFWwindow *window, int width, int height) {
 ...
 * @param window
 */
void reverse_triangle::processInput(GLFWwindow *window) {
  if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS) {
    glfwSetWindowShouldClose(window, true);
  }
}

void reverse_triangle::check_shader(unsigned int shader, const string &type) {
  int success;
  char infoLog[512];
...
void reverse_triangle::check_program(unsigned int shaderProgram) {
  int success;
  char infoLog[512];
...
void reverse_triangle::draw_frame(unsigned int shaderProgram, unsigned int VAO) {
