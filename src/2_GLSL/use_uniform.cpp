...
int use_uniform::create_a_triangle_with_glsl_uniform() {
  init_glfw();
...
GLFWwindow *use_uniform::create_glfw_window() {
  // Create a windows
...
void use_uniform::framebuffer_size_callback(GLFWwindow *window, int width, int height) {
  glViewport(0, 0, width, height);
}

void use_uniform::check_shader(unsigned int shader, const std::string &type) {
  int success;
  char infoLog[512];
...
void use_uniform::check_program(unsigned int shaderProgram) {
  int success;
  char infoLog[512];
...
void use_uniform::processInput(GLFWwindow *window) {
  if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS) {
    glfwSetWindowShouldClose(window, true);
  }
}

void use_uniform::draw_frame(unsigned int shaderProgram, unsigned int VAO) {
