...
#include "color_in_vertices.h"

#include <iostream>

using namespace std;

int color_in_vertices::create_a_triangle_color_in_vertices() {
    init_glfw();
...
void color_in_vertices::init_glfw() {
    // glfw initialize
    glfwInit();
...
GLFWwindow *color_in_vertices::create_glfw_window() {
    // Create a windows
...
void color_in_vertices::framebuffer_size_callback(GLFWwindow *window, int width, int height) {
    glViewport(0, 0, width, height);
}

void color_in_vertices::check_shader(unsigned int shader, const std::string &type) {
    int success;
    char infoLog[512];
...
void color_in_vertices::check_program(unsigned int shaderProgram) {
    int success;
    char infoLog[512];
...
void color_in_vertices::processInput(GLFWwindow *window) {
    if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS) {
        glfwSetWindowShouldClose(window, true);
    }
}

void color_in_vertices::draw_frame(unsigned int shaderProgram, unsigned int VAO) {
    // 设置调用 clear 所使用的颜色
